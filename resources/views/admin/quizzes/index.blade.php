...
<div class="container-fluid">
    <div class="row mb-5 align-items-center">
        <div class="col-lg-3 mb-4 mb-lg-0">
            <a href="{{route('admin.quizzes.create')}}" class="btn btn-primary light btn-lg btn-block rounded shadow px-2">+ Buat Kuis</a>
        </div>
    ...
    <div class="row">
        ...
        @forelse(\App\Kuis::all() as $quiz)
        <div class="col-lg-12">
            <div class="widget-stat card">
                <div class="card-body p-4">
                    <div class="media ai-icon">

                        @if($quiz->set_kuis()->count() > 0)
                        ...
                        <div class="media-body d-flex align-items-end justify-content-between">
                            <div>
                                <h4 class="mt-1">{{$quiz->name}}</h4>
                                <p class="mb-0">{{$quiz->soal()->count()}} Soal</p>
                            </div>

                            <div class="d-flex justify-content-end">
                                <a href="{{ route('admin.quizzes.questions_index', $quiz->id) }}"><span class="badge badge-primary">Soal</span></a>
                                <a href="{{ route('admin.quizzes.edit', $quiz->id) }}"><span class="badge badge-warning">Edit Kuis</span></a>
                                <a href="{{ route('admin.quizzes.delete', $quiz->id) }}" onclick="return confirm('Apakah anda yakin hapus data Kuis ini ?')"><span class="badge badge-danger">Hapus Kuis</span></a>
                                <a href="{{ route('admin.quizzes.schedules_index', $quiz->id) }}"><span class="badge badge-primary">Set Kuis</span></a>
                                <a href="{{ route('admin.quizzes.submissions_index', $quiz->id) }}"><span class="badge badge-success">( {{ $quiz->set_kuis()->count() }} ) Lihat Submit</span></a>
                            </div>
                        </div>
                    </div>
                </div>
