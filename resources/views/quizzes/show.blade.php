...
@php
$attachments = \App\Kuis::find($schedule->kuis_id)->attachments;
@endphp
...
<a class="schedule-event-inner d-flex justify-content-center align-items-center btn-soal d-lg-none"><i class="fa fa-eye fs-24" style="color: #fff;"></i></a>
    ...
    <div class="mx-5">
        <div class="row">
            ...
            <div class="col-lg-8">
                ...
                <form action="{{route('quizzes.submit', $schedule->id)}}" id="form-submit" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="">
                        <!-- Title -->
                        ...
                        @foreach(\App\AksesKuis::where('set_kuis_id',$schedule->id)->where('user_id',Auth::user()->id)->get() as $key => $question)
                                        ...
                                        <div class="custom-tab-1">
                                            <div id="profile-settings">
                                                <div class="pt-0">
                                                    ...
                                                    @if($question->soal->getfotoKuis() == TRUE)
                                                    <img src="{{$question->soal->getfotoKuis()}}" alt="" class="img-fluid mb-4" loading="lazy" style="height: auto; width: 100%; border-radius: 15px;">
                                                    @endif
                                                    ...
                                                    <div class="d-flex align-items-start justify-content-start">
                                                        ...
                                                        <div class="w-cont">
                                                            <h3 class="judul">{!! $question->soal->name !!}</h3>
                                                            <div class="settings-form">
                                                                <div class="radio d-flex align-items-center wans">
                                                                    <label class="form-control d-flex align-items-center" style="cursor: pointer;">
                                                                        <input type="radio" name="jawaban[{{$question->id}}]" class="mr-3 ml-2 ans" value="a" onclick="ahayyy('{{route('kuis.jawab.ajax',['type' => 'jawab','jawaban' =>'a','id' => $question->id])}}');" @if($question->jawaban == "a") checked @endif>
                                                                    <b>A .</b>&nbsp;{{$question->soal->a}}
                                                                    </label>
                                                                </div>
    
                                                                <div class="radio d-flex align-items-center wans">
                                                                    <label class="form-control d-flex align-items-center"  style="cursor: pointer;">
                                                                        <input type="radio" name="jawaban[{{$question->id}}]" class="mr-3 ml-2 ans" value="b" onclick="ahayyy('{{route('kuis.jawab.ajax',['type' => 'jawab','jawaban' =>'b','id' => $question->id])}}');" @if($question->jawaban == "b") checked @endif>
                                                                        <b>B .</b>&nbsp; {{$question->soal->b}}
                                                                    </label>
                                                                </div>
    
                                                                <div class="radio d-flex align-items-center wans">
                                                                    <label class="form-control d-flex align-items-center" style="cursor: pointer;">
                                                                        <input type="radio" name="jawaban[{{$question->id}}]" value="c"class="mr-3 ml-2 ans" onclick="ahayyy('{{route('kuis.jawab.ajax',['type' => 'jawab','jawaban' =>'c','id' => $question->id])}}');" @if($question->jawaban == "c") checked @endif>
                                                                        <b>C .</b>&nbsp; {{$question->soal->c}}
                                                                    </label>
                                                                </div>
    
                                                                <div class="radio d-flex align-items-center wans">
                                                                    <label class="form-control d-flex align-items-center" style="cursor: pointer;">
                                                                        <input type="radio" name="jawaban[{{$question->id}}]" class="mr-3 ml-2 ans" value="d" onclick="ahayyy('{{route('kuis.jawab.ajax',['type' => 'jawab','jawaban' =>'d','id' => $question->id])}}')" @if($question->jawaban == "d") checked @endif>
                                                                        <b>D .</b>&nbsp; {{$question->soal->d}}
                                                                    </label>
                                                                    <input type="hidden" name="isian[{{$question->id}}]" value="0">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="d-flex align-items-start justify-content-start">
                                                        ...
                                                        <div class="w-cont">
                                                            ...
                                                            <div class="fileInputContainer">
                                                                <input type="hidden" name="isian[{{$question->id}}]" value="1">
                                                                <input type="file" name="jawaban[{{$question->id}}]" id="{{$question->id}}" value="1" class="file" multiple="true">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
<div class="card container">
    ...
    <div class="card-body">
        ...
        <div class="previous-attachment mb-3">
            <a href="{{ config('app.quiz_attachments_url') . '/' .$attachment}}" download style="font-size: larger;"><i class="fa fa-download"></i> {{$attachment}}</a>
        </div>
        @endforeach
    </div>
</div>
...
<script>
        ...
        pond.setOptions({
            ...
            server: {
                process: function (fieldName, file, metadata, load, error, progress, abort, transfer, options) {
                    let postUrl = '{{ route("quizzes.upload_temp", $schedule->id) }}';
                    let patchUrl = '{{ route("quizzes.patch_temp", $schedule->id) }}';
                    postUrl += `?questionId=${inputElement.id}`;
                    patchUrl += `?questionId=${inputElement.id}`;
                            ...
                            abort: () => {
                                if (chunkRequests[postId]) {
                                    ...
                                    if (chunkRequests[postId].loaded > 0) {
                                        let deleteUrl = '{{ route("quizzes.delete_temp", $schedule->id) }}';
                                        deleteUrl += `?questionId=${inputElement.id}`;
                                        $.ajax({
                                            url: deleteUrl,
    ...
    const submitJawaban = function(instantSubmit = false) {
        ...
        if (instantSubmit) {
            $.ajax({
                type: "POST",
                url: "{{route('quizzes.submit', $schedule->id)}}",
                ...
                success: function(response){
                    window.location = "{{route('quizzes.index')}}"
                },
            });
    ...
    function load_no() {
        $(".nomor-soal").load("{{route('kuis.jawab.ajax',['type' => 'no-soal','setkuis' => $schedule->id])}}");
    }
    ...
    $waktu_kuis_fix = \Carbon\Carbon::parse($schedule->tanggal_mulai)->addMinutes($schedule->durasi);

    @endphp

    function makeTimer() {
            var endTime = new Date("{{\Carbon\Carbon::parse($schedule->tanggal_mulai)->addMinutes($schedule->durasi)->format('M d, Y H:i:s')}} UTC+07:00");
